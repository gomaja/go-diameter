...
func buildGyCCA() *Message {
	...
	mustGyAVP(m, avp.SessionID, avp.Mbit, 0, datatype.UTF8String("session;1234567890"))
	mustGyAVP(m, avp.OriginHost, avp.Mbit, 0, datatype.DiameterIdentity("ocs.example.com"))
	mustGyAVP(m, avp.OriginRealm, avp.Mbit, 0, datatype.DiameterIdentity("example.com"))
	mustGyAVP(m, avp.AuthApplicationID, avp.Mbit, 0, datatype.Unsigned32(4))
	mustGyAVP(m, avp.CCRequestType, avp.Mbit, 0, datatype.Enumerated(2)) // UPDATE_REQUEST
	mustGyAVP(m, avp.CCRequestNumber, avp.Mbit, 0, datatype.Unsigned32(1))
	mustGyAVP(m, avp.ResultCode, avp.Mbit, 0, datatype.Unsigned32(2001))

	// MSCC #1: Data service with volume grants (3 levels deep)
	mustGyAVP(m, avp.MultipleServicesCreditControl, avp.Mbit, 0, &GroupedAVP{
		AVP: []*AVP{
