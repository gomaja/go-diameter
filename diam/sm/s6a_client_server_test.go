...
func testHandleAIR(results chan error, settings *Settings) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		mustS6aAVP(a, avp.OriginHost, avp.Mbit, 0, settings.OriginHost)
...
func testSendAIA(w io.Writer, m *diam.Message) (n int64, err error) {
	...
	mustS6aAVP(m, avp.AuthenticationInfo, avp.Mbit, VENDOR_3GPP, &diam.GroupedAVP{
		AVP: []*diam.AVP{
...
func testHandleCLR(results chan error, settings *Settings) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		mustS6aAVP(a, avp.AuthSessionState, avp.Mbit, 0, datatype.Enumerated(clr.AuthSessionState))
		mustS6aAVP(a, avp.OriginHost, avp.Mbit, 0, settings.OriginHost)
		mustS6aAVP(a, avp.OriginRealm, avp.Mbit, 0, settings.OriginRealm)
...
func testHandleULR(results chan error, settings *Settings) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		// SessionID is required to be the AVP in position 1
		a.InsertAVP(diam.NewAVP(avp.SessionID, avp.Mbit, 0, req.SessionID))
		mustS6aAVP(a, avp.AuthSessionState, avp.Mbit, 0, req.AuthSessionState)
		mustS6aAVP(a, avp.OriginHost, avp.Mbit, 0, settings.OriginHost)
		mustS6aAVP(a, avp.OriginRealm, avp.Mbit, 0, settings.OriginRealm)
		mustS6aAVP(a, avp.OriginStateID, avp.Mbit, 0, settings.OriginStateID)
...
func testSendCLR(c diam.Conn, cfg *Settings) error {
	...
	mustS6aAVP(m, avp.SessionID, avp.Mbit, 0, datatype.UTF8String(sid))
	mustS6aAVP(m, avp.OriginHost, avp.Mbit, 0, cfg.OriginHost)
	mustS6aAVP(m, avp.OriginRealm, avp.Mbit, 0, cfg.OriginRealm)
	mustS6aAVP(m, avp.DestinationRealm, avp.Mbit, 0, meta.OriginRealm)
	mustS6aAVP(m, avp.DestinationHost, avp.Mbit, 0, meta.OriginHost)
	mustS6aAVP(m, avp.UserName, avp.Mbit, 0, datatype.UTF8String(TEST_IMSI))
	mustS6aAVP(m, avp.AuthSessionState, avp.Mbit, 0, datatype.Enumerated(0))
	mustS6aAVP(m, avp.CancellationType, avp.Mbit, 0, datatype.Enumerated(2))
	_, err := m.WriteTo(c)
	return err
}

func testSendULA(settings *Settings, w io.Writer, m *diam.Message) (n int64, err error) {
	mustS6aAVP(m, avp.ULAFlags, avp.Mbit|avp.Vbit, VENDOR_3GPP, datatype.Unsigned32(1))
	mustS6aAVP(m, avp.SubscriptionData, avp.Mbit, VENDOR_3GPP, &diam.GroupedAVP{
		AVP: []*diam.AVP{
...
func testSendAIR(c diam.Conn, cfg *Settings) error {
	...
	mustS6aAVP(m, avp.UserName, avp.Mbit, 0, datatype.UTF8String(TEST_IMSI))
	mustS6aAVP(m, avp.AuthSessionState, avp.Mbit, 0, datatype.Enumerated(0))
	mustS6aAVP(m, avp.VisitedPLMNID, avp.Vbit|avp.Mbit, uint32(VENDOR_3GPP), datatype.OctetString(PLMN_ID))
	mustS6aAVP(m, avp.RequestedEUTRANAuthenticationInfo, avp.Vbit|avp.Mbit, uint32(VENDOR_3GPP), &diam.GroupedAVP{
		AVP: []*diam.AVP{
			diam.NewAVP(
...
func testSendULR(c diam.Conn, cfg *Settings) error {
	...
	mustS6aAVP(m, avp.UserName, avp.Mbit, 0, datatype.UTF8String(TEST_IMSI))
	mustS6aAVP(m, avp.AuthSessionState, avp.Mbit, 0, datatype.Enumerated(0))
	mustS6aAVP(m, avp.RATType, avp.Mbit, uint32(VENDOR_3GPP), datatype.Enumerated(1004))
	mustS6aAVP(m, avp.ULRFlags, avp.Vbit|avp.Mbit, uint32(VENDOR_3GPP), datatype.Unsigned32(ULR_FLAGS))
	mustS6aAVP(m, avp.VisitedPLMNID, avp.Vbit|avp.Mbit, uint32(VENDOR_3GPP), datatype.OctetString(PLMN_ID))
	_, err := m.WriteTo(c)
	return err
}
