...
func (p *Parser) FindAVPByCode(appid, code, vendorID uint32) (*AVP, error) {
	...
	// Base AVPs are common across Diameter messages, so decode vendor-id 0
	// base AVPs even when the application dictionary is not loaded (RFC 6733 §2).
	if appid != 0 && vendorID == 0 {
		if avp, ok := p.avpcode[codeIdx{0, code, 0}]; ok {
			return avp, nil
		}
	}
