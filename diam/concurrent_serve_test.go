...
func serveTestServer(tb testing.TB, srv *Server, ln net.Listener) {
	tb.Helper()
	...
	tb.Cleanup(func() {
		...
		if err := <-errc; err != nil && !errors.Is(err, ErrServerClosed) {
			tb.Errorf("serve: %v", err)
		}
	})
}

func closeTestConn(tb testing.TB, conn net.Conn) {
	tb.Helper()
...
func runDispatchBenchmark(b *testing.B, maxConcurrent int, handlerLatency time.Duration, total int) {
	...
	for i := 0; i < b.N; i++ {
		...
		conn, err := net.Dial("tcp", ln.Addr().String())
		if err != nil {
			if closeErr := srv.Close(); closeErr != nil && !errors.Is(closeErr, ErrServerClosed) {
				b.Fatalf("server close: %v", closeErr)
			}
			if serveErr := <-serveErr; serveErr != nil && !errors.Is(serveErr, ErrServerClosed) {
				b.Fatalf("serve: %v", serveErr)
			}
			b.Fatal(err)
		}
