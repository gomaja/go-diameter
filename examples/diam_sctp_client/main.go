...
func sendCER(w io.Writer) (n int64, err error) {
	...
	if _, err := m.NewAVP(avp.OriginHost, avp.Mbit, 0, datatype.OctetString(*host)); err != nil {
		return 0, err
	}
	if _, err := m.NewAVP(avp.OriginRealm, avp.Mbit, 0, datatype.OctetString(*realm)); err != nil {
		return 0, err
	}
	if _, err := m.NewAVP(
		avp.HostIPAddress,
	...
	if _, err := m.NewAVP(avp.OriginStateID, avp.Mbit, 0, originStateID); err != nil {
		return 0, err
	}
	if _, err := m.NewAVP(avp.AcctApplicationID, avp.Mbit, 0, datatype.Unsigned32(1)); err != nil {
		return 0, err
	}
	if _, err := m.NewAVP(avp.VendorSpecificApplicationID, avp.Mbit, 0, &diam.GroupedAVP{
		AVP: []*diam.AVP{
...
func handleDWR(errc chan error) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		if _, err := a.NewAVP(avp.OriginHost, avp.Mbit, 0, datatype.OctetString(*host)); err != nil {
			errc <- err
			return
		}
		if _, err := a.NewAVP(avp.OriginRealm, avp.Mbit, 0, datatype.OctetString(*realm)); err != nil {
			errc <- err
			return
		}
		if _, err := a.NewAVP(avp.OriginStateID, avp.Mbit, 0, originStateID); err != nil {
			errc <- err
			return
		}
		_, err = a.WriteTo(c)
