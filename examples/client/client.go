...
func main() {
	...
	cli := &sm.Client{
		...
		AuthApplicationID: []*diam.AVP{
			...
			diam.NewAVP(avp.AuthApplicationID, avp.Mbit, 0, datatype.Unsigned32(4)), // RFC 8506
		},
	}
...
func sendHMR(c diam.Conn, cfg *sm.Settings) error {
	...
	if err := addAVP(m, avp.UserName, avp.Mbit, 0, datatype.UTF8String("foobar")); err != nil {
		return err
	}
...
func sendACR(c diam.Conn, cfg *sm.Settings, n int) {
	...
	for i := 0; i < n; i++ {
		...
		if err := addAVP(m, avp.SessionID, avp.Mbit, 0, datatype.UTF8String(strconv.Itoa(i))); err != nil {
			log.Fatal(err)
		}
		if err := addAVP(m, avp.OriginHost, avp.Mbit, 0, cfg.OriginHost); err != nil {
			log.Fatal(err)
		...
		if err := addAVP(m, avp.AccountingRecordType, avp.Mbit, 0, eventRecord); err != nil {
			log.Fatal(err)
		}
		if err := addAVP(m, avp.AccountingRecordNumber, avp.Mbit, 0, datatype.Unsigned32(i)); err != nil {
			log.Fatal(err)
		}
		if err := addAVP(m, avp.DestinationHost, avp.Mbit, 0, meta.OriginHost); err != nil {
			log.Fatal(err)
