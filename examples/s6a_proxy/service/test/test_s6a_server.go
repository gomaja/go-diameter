...
import (
	...
	"github.com/gomaja/go-diameter/examples/s6a_proxy/service"
)
...
func testHandleAIR(settings *sm.Settings) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		if err := testAddAVP(a, avp.OriginHost, avp.Mbit, 0, settings.OriginHost); err != nil {
			fmt.Printf("failed to build AIA: %s", err)
			return
		}
		if err := testAddAVP(a, avp.OriginRealm, avp.Mbit, 0, settings.OriginRealm); err != nil {
			fmt.Printf("failed to build AIA: %s", err)
			return
		}
		if err := testAddAVP(a, avp.OriginStateID, avp.Mbit, 0, settings.OriginStateID); err != nil {
			fmt.Printf("failed to build AIA: %s", err)
			return
		}
...
func testSendAIA(w io.Writer, m *diam.Message, vectors int) (int64, error) {
	...
	for ; vectors > 0; vectors-- {
		if err := testAddAVP(m, avp.AuthenticationInfo, avp.Mbit, service.VENDOR_3GPP, &diam.GroupedAVP{
			AVP: []*diam.AVP{
...
func testHandleULR(settings *sm.Settings) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		// SessionID is required to be the AVP in position 1
		a.InsertAVP(diam.NewAVP(avp.SessionID, avp.Mbit, 0, req.SessionID))
		if err := testAddAVP(a, avp.AuthSessionState, avp.Mbit, 0, req.AuthSessionState); err != nil {
			fmt.Printf("failed to build ULA: %s", err)
			return
		}
		if err := testAddAVP(a, avp.OriginHost, avp.Mbit, 0, settings.OriginHost); err != nil {
			fmt.Printf("failed to build ULA: %s", err)
...
func testSendULA(settings *sm.Settings, w io.Writer, m *diam.Message) (int64, error) {
	if err := testAddAVP(m, avp.ULAFlags, avp.Mbit|avp.Vbit, service.VENDOR_3GPP, datatype.Unsigned32(1)); err != nil {
		return 0, err
	}
	if err := testAddAVP(m, avp.SubscriptionData, avp.Mbit, service.VENDOR_3GPP, &diam.GroupedAVP{
		AVP: []*diam.AVP{
