...
func handleHMR(silent bool) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		if err := addAVP(a, avp.SessionID, avp.Mbit, 0, hmr.SessionID); err != nil {
			log.Printf("Failed to create Session-Id AVP: %s", err.Error())
			return
		}
		if err := addAVP(a, avp.OriginHost, avp.Mbit, 0, hmr.DestinationHost); err != nil {
			log.Printf("Failed to create Origin-Host AVP: %s", err.Error())
			return
		}
		if err := addAVP(a, avp.OriginRealm, avp.Mbit, 0, hmr.DestinationRealm); err != nil {
			log.Printf("Failed to create Origin-Realm AVP: %s", err.Error())
			return
		}
		if err := addAVP(a, avp.DestinationRealm, avp.Mbit, 0, hmr.OriginRealm); err != nil {
			log.Printf("Failed to create Destination-Realm AVP: %s", err.Error())
			return
		}
		if err := addAVP(a, avp.DestinationHost, avp.Mbit, 0, hmr.OriginHost); err != nil {
			log.Printf("Failed to create Destination-Host AVP: %s", err.Error())
			return
		}
		_, err := a.WriteTo(c)
		if err != nil {
...
func handleACR(silent bool) diam.HandlerFunc {
	...
	return func(c diam.Conn, m *diam.Message) {
		...
		if err := addAVP(a, avp.OriginHost, avp.Mbit, 0, acr.DestinationHost); err != nil {
			log.Printf("Failed to create Origin-Host AVP: %s", err.Error())
			return
		}
		if err := addAVP(a, avp.OriginRealm, avp.Mbit, 0, acr.DestinationRealm); err != nil {
			log.Printf("Failed to create Origin-Realm AVP: %s", err.Error())
			return
		}
